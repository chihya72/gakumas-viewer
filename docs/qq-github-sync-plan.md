...
## 1.5 当前状态速览（2026-07-23）

阶段 0～6 全部交付。QQ 与网页双向写入同一个 GitHub 工作仓库；产物、备份、记录、
校对 TXT 一次提交完成；旧稿不覆盖新稿；断网不丢稿；草稿可续作；身份跨端互认。
两端 `records` 逐字段一致（325 条，指纹相同）。

规模：325 个工单／记录，正式产物 849 =
`raw_txt 248 + ai_csv 110 + translated_csv 268 + proofread_csv 223`。

### 五条不变量

改动任何一端之前，先确认没有破坏这五条。它们每一条都对应一次真实事故。

1. **`file_id` 只来自 Issue 标题。** 不从路径、文件名反推——
   `proofread_csv/adv/pstory/003/hski/final-failure-01.csv` 的 basename 只是
   `final-failure-01`，据此写记录会落到不存在的工单上。
2. **轨道标记存鉴权身份**（有 GitHub 账号写 login，否则写 `qq-<号>`），
   个人 ID 只用于显示。个人 ID 会被改名，用它做判定会让历史标记集体变成孤儿。
3. **时间戳一律 UTC、秒级、`Z` 结尾。** 带时区偏移的字符串比较会把
   `+08:00` 判成比同一时刻的 `Z` 更晚。
4. **除入库时间清单外，一律 API 直读并破代理缓存。**
   `raw.githubusercontent` 无视查询串，用它取「要用的内容」会拿到旧版本。
5. **`records/<file_id>.json` 是完成状态的唯一真源**，Issue 与
   `compare_status.csv` 都是投影。投影必须能从真源整体重建，不能只加不减。

### 排查手册

| 症状 | 首先怀疑 | 怎么确认 |
| --- | --- | --- |
| 保存/完成报「分支已被推进」 | 读 HEAD 命中了代理缓存 | 看仓库最近提交时间——若上一次就是自己刚提交的，就是缓存 |
| 打开刚完成的文件被判「他人已更新」 | 判定与提交读了不同新鲜度的数据源 | 用 API 直读记录对比页面显示的版本号 |
| 工作台没有「开始翻译」等按钮 | 身份比较没做归一 | 看 Issue 轨道里存的是 login 还是个人 ID |
| 记录写到了奇怪的文件名上 | `file_id` 来源不是 Issue 标题 | `npm run audit:work` 的「缺记录」与记录数会露馅 |
| QQ 端状态不随网页更新 | 只改了 Issue，没写记录 JSON | Bot 游标是 git HEAD，改 Issue 不产生 commit |
| 本地比 GitHub 多出文件 | 删除没有传播 | 比对两端 `records` 数量与指纹 |
| 机翻稿里出现人名署名 | 有写入路径回写了「打开的那个文件」 | 查该 `ai_csv` 的提交历史里有没有「更新翻译」 |

### 两个可跑的检查

- `npm run check:stage` —— 完成事务、草稿、身份归一的 56 项断言，假 wrapper 实跑，不碰网络。
- `npm run audit:work` —— 工作仓库全库体检（只读，需 `gh` 已登录），
  输出记录指纹，与 NAS 侧同算法结果比对即可判断两端是否逐字段一致。

已知且确认不处理：`adv_dear_hmsz_026`、`adv_dear_ttmr_029` 的校对时间早于翻译时间
（翻译在校对之后又提交过一次）；14 个 `ai_csv` 的署名是跑机翻的人而非 AI 标识。
...
### 实施进度流水（按时间累积，供追溯用；当前状态见第 1.5 节）
