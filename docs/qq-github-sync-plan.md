...
1. **Bot 侧投影层修正**（NAS，最优先）：镜像传播删除、`compare_status.csv` 改为全量重算、
   写入前校验 `file_id`。这三条的偏差单调累积、不会自愈，且已经真实发生过（幽灵记录）。
   顺带把 Bot 自己的镜像备份目录改名，避开协议已占用的 `*_backup/`。
2. **补齐 outbox 与重试**（阶段 6，NAS 侧）：GitHub 暂时不可用时保留本地待同步任务，
   恢复后按 commit/哈希幂等重试。这是唯一完全未实现的验收项。
3. **清理旧投影流程**（阶段 7）：删除 `updateContent` 的单文件重试路径，
   把仅剩的调用点迁到 `commitFiles`；补做断网、重启、非法 CSV 三类测试。
   （编辑器回写 `ai_csv` 一项已在 2026-07-22 修复并还原了受影响的 15 个机翻稿。）
4. **原文或机翻更新后标记 `stale` / `review_needed`**（验收第 12 条，未实现，优先级最低）：
   当前只有两个文件因翻译在校对后重交而处于这种状态，已确认不处理。
...
- **判定用 API 直读并破代理缓存，显示用 raw 缓存**。凡是拿来做决定的（能否编辑、
  版本号、草稿归属），一律 API；只用于展示的容忍延迟。
