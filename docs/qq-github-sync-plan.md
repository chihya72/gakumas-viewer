...
当前 GitHub 工作仓库 HEAD：`92971d1750e9a3f0e5cf7bf3b66e46331ba4ed96`
...
| Bot 文件镜像 | `artifact_sync_complete=true`，848 个产物已追平（仅同步 `raw_txt`／`ai_csv`／`translated_csv`／`proofread_csv`）|
...
| 2 | 网页中途保存，Bot 能拉到草稿 | 未验 | 草稿目录尚无文件；Bot 已明确不把草稿并入正式镜像 |
| 3 | 网页完成后，Bot 能获得正式翻译和备份 | 部分 | 完成事务已自检；Bot 侧备份单独保留，尚无实测 |
| 4 | 同名上传只保留最新一份显式备份 | 自检通过 | 「备份只有一份」「备份是上一版」 |
| 5 | 两人同时完成同一文件，只有一个成功 | 自检通过 | 「旧稿被拒绝」「拒绝时未产生提交」 |
| 6 | 旧版本文件不会覆盖新正式稿 | 自检通过 | 同上，`StaleRevisionError` |
| 7 | 重复提交不重复增加版本 | 自检通过 | 「内容相同不涨版本」（协议第 7 节幂等） |
| 8 | Bot 写文件中途崩溃后可恢复 | 未验 | 属阶段 6 |
| 9 | GitHub 提交成功但本地写入失败可重放 | 未验 | 属阶段 6 |
| 10 | 本地成功但 GitHub 不可用，outbox 重试 | **未实现** | 属阶段 6 |
| 11 | 未注册 QQ／错误 login／群名片变化不能绕过鉴权 | 部分 | 身份归一已自检 9 项；群名片本就不参与判定 |
| 12 | 原文或机翻更新后旧译文标记过时 | **未实现** | 无 `stale` / `review_needed` 机制 |
| 13 | 草稿存在时能恢复正确的有效草稿 | 自检通过 | 归属、过期、完成后删除共 8 项断言 |
| 14 | 缺失文件／错误 SHA／非法 CSV 不写入完成状态 | 部分 | HTML 标签校验与日语原文一致性校验已有；SHA 校验在 Bot 侧 |

### 全库体检（2026-07-23，HEAD `92971d1`）
...
| 完成但缺产物 ／ 记录残留本地路径 | 0 ／ 0 |
| 正式镜像 | 248 + 110 + 268 + 222 = **848**，与 Bot `artifact_expected_count` 一致 |
| 草稿 ／ 备份 ／ 校对 TXT | 0 ／ 0 ／ 0（尚未产生，Bot 已排除出正式镜像） |
...
1～4 项（文件镜像、草稿层、版本冲突保护、正式稿原子轮换）已于 2026-07-22～23 交付，
详见第 16 节路线图与第 17 节验收表。剩下三件：

1. **草稿与备份的首次实战核对**：这两类目录目前是空的，Bot 已把它们排除出正式镜像，
   但排除逻辑尚未经受真实文件检验。等第一次「中途保存」和第一次覆盖式完成发生后，
   重跑两端指纹与 `artifact_expected_count` 核对。
2. **补齐 outbox 与重试**（阶段 6，NAS 侧）：GitHub 暂时不可用时保留本地待同步任务，
   恢复后按 commit/哈希幂等重试。这是目前唯一完全未实现的验收项。
3. **清理旧投影流程并验收**（阶段 7）：编辑器不再把中途内容写回 `ai_csv`、
   删除 `updateContent` 的单文件重试路径；补做断网、重启、非法 CSV 三类测试。

另有一项未实现但优先级低：原文或机翻更新后把旧译文标记为 `stale` / `review_needed`
（验收第 12 条）。当前只有两个文件因翻译在校对后重交而处于这种状态，已确认不处理。
