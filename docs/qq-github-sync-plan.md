...
| 翻译/校对草稿 | 目前均为 0 个 |
| 翻译/校对备份 | 目前均为 0 个 |
| `source_times.json` | 已生成，325 条，冷启动入库时间请求 215 → 1 |
...
- [ ] 待验证：QQ 认领/上传这条常规路径写出的时间戳（尚无写入样本；为避免扰乱真实排期未做占坑测试）。风险低——记录写入都经 `now_iso()`，除非该路径绕开记录层单独取时间。
...
| 顺序 | 事项 | 位置 | 完成判据 |
| --- | --- | --- | --- |
| ~~1~~ | ~~统一时间戳写入为 UTC `Z`~~ 已完成 | NAS 插件 | 全库 1437 个时间字段严格匹配 `…SSZ`；仅剩常规 QQ 认领路径无写入样本 |
